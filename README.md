...
- docker-compose.yml: 전체 서비스(Nginx + Django 3개) 구성 및 실행 정의
...
## 주요 기능
- Round Robin 로드 밸런싱: Nginx 설정을 통해 요청이 3개의 서버에 순차적으로 분산됩니다.
- 컨테이너 식별: socket.gethostname()을 통해 각 응답이 어느 컨테이너에서 생성되었는지 추적 가능합니다.
- 컨테이너화: 모든 환경이 Docker로 고립되어 있어 어디서든 동일하게 실행됩니다.

## 향후 계획
- CrateDB 클러스터(Master 1, Data 2) 연동 및 데이터베이스 클러스터링 테스트